...
library(tidyverse)       #package for languange and syntax that makes code a little more friendly
                         #includes a host of other packages like dplyr, ggplot2, etc.
...
LILA.egg.counts <- read_excel("EggMassData_v1.12_nb.xlsx", sheet = 2) %>% 
...
####create a data frame of transect areas see metadata in EggMassData_v1.12_nb####

#line 54 -> create a list naming the cells each transect is in
#line 55 -> create a list namint the transects in each cell
#line 56 -> create a list of the areas in square meters of each transect
...
head(transectarea)  #check beggining of transect areas
tail(transectarea)  #check end of transect areas

###add the transect areas to the LILA.egg.count dataframe
...
#line 78 -> read in Environmental Depth Data, on sheet two of our excel file, we need to skip
#the fist 6 lines that are describing the transponders
#line 79 -> Change the variable name from Station to Cell
#line 80 -> Change the variable name from 'Daily Date' to Date
#line 81 -> Change the variable name from 'Data Value' to Depth.ft

depth.raw <- read_excel("LILA_EnvironmentalData_060118-081021.xlsx", sheet = 2, skip = 6) %>% 
  rename(Cell = Station,
         Date = 'Daily Date',
         Depth.ft = 'Data Value') 

#line 89 -> save our changes to depth raw
#line 90-96 -> change all the tansponder names to wetland names
#line 97-> add a new variable called depth.cm converting stage feet to depth above deep slough
#in cm
#line 98-100-> add a new variable called treatment that characterizes the treatments
...
###check the new data
table(depth.raw$Cell)          #looks good        
table(depth.raw$treatment)     #looks good
hist(depth.raw$depth.cm)       #looks good

####load in Temperature data #####

#line 112-113 -> read in temperature data using our third sheet skipping the transponder description
#line 114-114 -> change 'Daily Date' and 'Data Value' to Date and Temp.c

temp.raw <- read_excel("LILA_EnvironmentalData_060118-081021.xlsx", sheet = 4,
                        skip = 3) %>% 
...
#lines 136 -> save our changes 
#lines 137-139 -> create a new variable with individual dates have the same monday date
#lines 140 -> use the new variable and Cell as groups
#lines 141 -> average our depths to get weekly averages for each Cell
#lines 142 -> initiate a plot, with week.date on the x access, and depth.cm on the y, 
#with color groups by Cell
#lines 143 -> use theme classic to have white background 
#lines 144 -> add lines to our plot from our data
#lines 145-146 -> Add Labels to our plot
#lines 147-150 -> first line rotates x axis labels to be 60 degrees, other lines
#adjust text sizes
#lines 151-152-> format the x axis labels to be every month, and format the labels to be
# month year with the month abreviated and year eg "Jan. 18"
#lines 153-154 -> specify specific colors for each Cell and remove legent title
...
####save our plot as a .png with the width and height dimension in inches

ggsave("Hydrograph.png", plot = last_plot(), device = "png", width = 6.66, height = 3.7)

#------------------------------------------------------------------------------------------
#####Get Model Counts and Covariates####
#------------------------------------------------------------------------------------------

#####Get Counts for Modelling#####

#lines 173 -> create a new data frame for our modelling data 
#lines 174 -> only use transect one and two as these are the deep slough-ridge transects
#lines 175 -> group everything together to combine transects
#lines 176-177 -> summ the two tansects counts and areas together
#lines 178 -> reformate our date from Poisix to Date sturcture, 
#lines 179 -> convert counts to density (meters squared)
...
hist(summ.LILA.egg$count)                #again the distribution is non-normal
hist(summ.LILA.egg$density)              #same for desnity

###lets look at our data using a time series

#lines 202 -> use our data from above both transect deep slough transects
#lines 203 -> use day of year as x axis, density on y, and shape and color as species
#lines 204 -> create a grid of plots with cell and year
#lines 205 -> use theme classic to get white background
#lines 206 -> add our points
#lines 207 -> scale our x axis and format the text to be days and months
#lines 208 -> add points to the lines

summ.LILA.egg %>%
  ggplot(aes(x = dayofyear, y = density, color = Pomacea_Species, shape = Pomacea_Species))+
...
####summarise depths to get weekly sd.depth and ave.depths####
...
hist(depth.summ$sd.depth.cm)      #interestind sd distribution

###add the data to our modelling count data
#lines 247 -> save our changes
#lines 248-250 -> create a week.date to put some dates on first of the week for adding data
#lines 251 -> combine the depth data using our weeks, now every point as a depth and sd from the 
#week prior

summ.LILA.egg <- summ.LILA.egg %>% 
  mutate(week.date = floor_date(Date,
                                  unit = "week",
                                  week_start  = getOption("lubridate.week.start", 1))) %>% 
  left_join(depth.summ, by = c("week.date","Cell"))

####summarise temperature to get weekly average temp####
...
#check data
hist(temp.summ$ave.temp.c)      #intesting our temperate is schewed

###add the data to our count dataframe

#lines 276 -> save it to our count dataframe
#lines 277 -> combine the temp data by week.date now our data has average air temperature
#from the week prior to our count
...
#lines 284-306 -> Seperate all the depths to each Cells
...
# create a difference in depths lagged by 21 days
...
#add the change covariate to our count data
...
TTdata <- read_excel("LILA_Stage_Contrast_TTdata_WS2018-DS2021.xlsx", sheet = 4)
...
summ_yearTT <- summ.LILA.egg %>% 
  ungroup() %>% 
  group_by(Pomacea_Species, year) %>% 
  summarise(pom.dens = mean(pom.dens, na.rm = T))
