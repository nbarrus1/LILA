...
# make sure all seasons of data that you eant to use in your analysis are included below
               ...
               CPUE_2021_SU, CPUE_2022_SP, CPUE_2022_SU)
View(catch)
...
###############   Run model for Hydropattern*Time

model.LPFA_tt <- lme(Predfishcount ~ Hydropattern + Cumulative + Hydropattern*Cumulative,
                     random = ~1|Macrocosm,
                     data = catch_standardized)
...
model.tt <- lme(Predfishcount ~ factor(Hydropattern)*ordered(Cumulative), 
                random = ~1|Macrocosm, correlation = corAR1(form = ~1|Macrocosm, value = ACF_tt_1, ACF_tt_2),
                data = catch_standardized, method = "REML")
# Ran all iterations of ACF values within the model (not included in code)
# two AIC value outputs came from simple (1 ACF value) and complex (2 ACF values) 
# Simple AIC = 103.502 and Complex AIC = 101.7286
...
#                                          numDF denDF   F-value p-value
# (Intercept)                                  1    12 140.97742  <.0001
# factor(Hydropattern)                         1     2   5.04955  0.1537
# ordered(Cumulative)                          6    12   3.99194  0.0198
# factor(Hydropattern):ordered(Cumulative)     6    12   2.24182  0.1103

# Check for violations of assumptions
# Assumption Testing:
# Assumption 1: Linearity of data (Explanatory variables are related linearly to the response variable/s)
#         Explained: plot residuals, if there is obvious patterning in the plot this assumption is VIOLATED!!!
plot.model.tw <- plot(resid(model.tt))


#Check assumption of homogeneity of variance:
catch_standardized$restt <- residuals(model.tt) #extracts residual and places them in a new column in dataframe
catch_standardized$absrestt <- abs(catch_standardized$restt) #creates new column with absolute value of residuals
catch_standardized$restt2 <- catch_standardized$absrestt^2 #squares ths absolute value of residuals to provide more robust estimate
Levene.model.tt <- lm(restt2 ~ Macrocosm, data = catch_standardized) #anova of the squared residuals
anova(Levene.model.tt) # P = 0.4458


# Test for normal distribution of residuals
shapiro.test(residuals(model.tt)) # P = 0.999
qqnorm(model.tt$residuals)
qqline(model.tt$residuals)
anova(model.tt)
